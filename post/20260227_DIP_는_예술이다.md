...
하지만 지금은 다르다. 명확한 요구사항과 설계가 있다면, AI는 보일러플레이트 코드를 순식간에 작성한다. [이전 글에서 다뤘던 Mermaid 다이어그램](./20260213_설계를_위한_요구사항은_없다.md)처럼, 클래스 다이어그램 하나를 보여주면 AI는 그에 맞는 코드 구조를 만들어낸다.
...
[테스트 글](./20260206_테스트_묻고_doubles_로_가.md)에서 언급했듯, Fake Repository나 In-memory 구현이 가능한 것 자체가 DIP가 제대로 적용되었다는 증거다. 인터페이스에 의존하지 않으면, 테스트를 위한 대체 구현이 존재할 수 없다.
...
> - [설계를 위한 요구사항은 없다](./20260213_설계를_위한_요구사항은_없다.md)
> - [테스트, 묻고 doubles로 가](./20260206_테스트_묻고_doubles_로_가.md)
